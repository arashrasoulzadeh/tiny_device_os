...
typedef struct hal_pwm {
    char path[32];
    ledc_channel_config_t channels[LEDC_MAX_CHANNELS];
    ledc_timer_config_t timers[4];
    bool channel_used[LEDC_MAX_CHANNELS];
    bool timer_used[4];
    bool initialized;
} hal_pwm_t;
...
static ledc_channel_t hal_to_esp_channel(const char* path) {
    ...
    if (channel >= 0 && channel < LEDC_MAX_CHANNELS) return channel;
...
void hal_pwm_close(hal_pwm_t* pwm) {
    ...
    if (pwm->initialized) {
        ...
        for (int i = 0; i < 4; i++) {
            if (pwm->timer_used[i]) {
                // Timer cleanup not needed explicitly
            }
        }
    }
    free(pwm);
...
int hal_pwm_set_frequency(hal_pwm_t* pwm, int channel, uint32_t freq) {
    if (!pwm || channel < 0 || channel >= LEDC_MAX_CHANNELS) return -1;
...
void hal_pwm_remove_channel(hal_pwm_t* pwm, int channel) {
    if (!pwm || channel < 0 || channel >= LEDC_MAX_CHANNELS) return;
