...
static i2s_comm_format_t hal_to_i2s_format(hal_audio_format_t format) {
    switch (format) {
        case HAL_AUDIO_FORMAT_PCM_S16_LE: return I2S_COMM_FORMAT_STAND_I2S;
        case HAL_AUDIO_FORMAT_PCM_S24_LE: return I2S_COMM_FORMAT_STAND_I2S;
        case HAL_AUDIO_FORMAT_PCM_S32_LE: return I2S_COMM_FORMAT_STAND_I2S;
        default: return I2S_COMM_FORMAT_STAND_I2S;
    }
}
...
int hal_audio_init(hal_audio_t* audio) {
    ...
    audio->i2s_config = (i2s_config_t){
        ...
        .bits_per_sample = hal_to_i2s_bits(audio->config.format),
...
int hal_audio_write(hal_audio_t* audio, const void* buffer, size_t frames) {
    ...
    esp_err_t err = i2s_write(I2S_NUM, buffer, frames * audio->config.channels * (hal_to_i2s_bits(audio->config.format) / 8), 
                              &bytes_written, pdMS_TO_TICKS(100));
    
    return err == ESP_OK ? (int)(bytes_written / (audio->config.channels * (hal_to_i2s_bits(audio->config.format) / 8))) : -1;
}
...
int hal_audio_read(hal_audio_t* audio, void* buffer, size_t frames) {
    ...
    size_t bytes_read = 0;
    esp_err_t err = i2s_read(I2S_NUM, buffer, frames * audio->config.channels * (hal_to_i2s_bits(audio->config.format) / 8),
                             &bytes_read, pdMS_TO_TICKS(100));
    
    return err == ESP_OK ? (int)(bytes_read / (audio->config.channels * (hal_to_i2s_bits(audio->config.format) / 8))) : -1;
}
