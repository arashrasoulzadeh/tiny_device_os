...
static i2c_mode_t hal_to_esp_mode(hal_i2c_speed_t speed) {
    return I2C_MODE_MASTER;
}

static int hal_to_esp_speed(hal_i2c_speed_t speed) {
    return speed;
}
...
hal_i2c_t* hal_i2c_open(const char* path, hal_i2c_speed_t speed) {
    ...
    if (strstr(path, "i2c1") || strstr(path, "i2c_1")) {
        port = I2C_NUM_1;
    } else if (strstr(path, "i2c0") || strstr(path, "i2c_0")) {
        port = I2C_NUM_0;
    }
    ...
    i2c->port = port;
    i2c->speed = speed;
    i2c->timeout_ms = I2C_TIMEOUT_MS_DEFAULT;
...
int hal_i2c_init_bus(hal_i2c_t* i2c) {
    ...
    i2c_config_t config = {
        ...
        .sda_io_num = (i2c->port == I2C_NUM_0) ? GPIO_NUM_21 : GPIO_NUM_13,
        .scl_io_num = (i2c->port == I2C_NUM_0) ? GPIO_NUM_22 : GPIO_NUM_14,
