...
typedef struct hal_spi {
    ...
    spi_device_handle_t devices[8];
    uint8_t device_count;
    bool initialized;
} hal_spi_t;

static spi_host_device_t hal_to_esp_host(const char* path) {
    if (strstr(path, "spi1") || strstr(path, "spi_1")) return SPI_HOST;
    if (strstr(path, "spi2") || strstr(path, "spi_2")) return HSPI_HOST;
    if (strstr(path, "spi3") || strstr(path, "spi_3")) return VSPI_HOST;
    return HSPI_HOST;
}
...
int hal_spi_init_bus(hal_spi_t* spi) {
    ...
    spi_bus_config_t bus_config = {
        .mosi_io_num = (spi->host == HSPI_HOST) ? GPIO_NUM_13 : GPIO_NUM_23,
        .miso_io_num = (spi->host == HSPI_HOST) ? GPIO_NUM_12 : GPIO_NUM_19,
        .sclk_io_num = (spi->host == HSPI_HOST) ? GPIO_NUM_14 : GPIO_NUM_18,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
...
int hal_spi_add_device(hal_spi_t* spi, const hal_spi_config_t* config, uint8_t cs_gpio, spi_device_handle_t* handle) {
    if (!spi || spi->device_count >= 8) return -1;
